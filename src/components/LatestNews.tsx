...
export default function LatestNews({ articles }: { articles: NewsArticleSummary[] }) {
  ...
  const categoryStyles: Record<string, { bg: string; label: string }> = {
    chronicle: { bg: "bg-[#EDBB00]/10 text-[#A50044]", label: t("news.matchReport") },
    digest: { bg: "bg-blue-50 text-blue-700", label: t("news.newsDigest") },
  };
    ...
    <section className="section-padding">
      <div className="container-custom">
        <div className="text-center mb-12">
          <h2 className="text-3xl md:text-4xl font-heading font-extrabold text-[#1A1A2E] mb-4">{t("section.latestNews")}</h2>
          <p className="text-gray-500">{t("section.latestNewsDesc")}</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {articles.slice(0, 6).map(article => {
            const style = categoryStyles[article.category] || categoryStyles.digest;
            return (
              <Link key={article.id} href={lp(`/news/${article.slug}`)} className="card group block">
                ...
                <div className="p-5">
                  ...
                  <div className="flex items-center justify-between text-xs text-gray-400">
                    ...
                    <span>{new Date(article.publishedAt).toLocaleDateString(locale === "es" ? "es-ES" : "en-US", { month: "short", day: "numeric", year: "numeric" })}</span>
                  </div>
                </div>
              </Link>
            );
        ...
        <div className="text-center mt-10">
          <Link href={lp("/news")} className="btn-secondary">{t("blog.readMore")}</Link>
        </div>
      </div>
    </section>
  );
