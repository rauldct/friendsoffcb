...
export default function PackageHero({ matchTitle: matchTitleEn, matchTitleEs, competition, matchDate, matchTime, heroImage }: Props) {
  const { t, locale } = useLanguage();
  const matchTitle = localized(locale, matchTitleEn, matchTitleEs);
  ...
  return (
    <section className="relative py-20 bg-gradient-to-br from-[#1A1A2E] via-[#004D98] to-[#A50044] overflow-hidden">
      ...
      <div className="container-custom text-center text-white relative z-10">
        <div className="mb-4 [&_a]:text-gray-300 [&_a:hover]:text-white [&_span]:text-gray-400 [&_nav]:text-gray-400">
          <Breadcrumbs items={[
        ...
        <p className="text-gray-300 mb-2">
          {new Date(matchDate).toLocaleDateString(locale === "es" ? "es-ES" : "en-US",{weekday:"long",month:"long",day:"numeric",year:"numeric"})} · {matchTime}
        </p>
        <p className="text-gray-400 mb-8">{t("misc.campNou")}</p>
        <div className="flex justify-center gap-6">
          {[{v:countdown.days,l:t("misc.days")},{v:countdown.hours,l:t("misc.hours")},{v:countdown.minutes,l:t("misc.min")}].map(c=>(
